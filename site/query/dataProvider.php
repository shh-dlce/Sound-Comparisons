...
class DataProvider {
  ...
  */
  public static function editTranscriptionTable($d){
    ...
    $q   = "SELECT DISTINCT t.*, l.FilePathPart, l.ShortName, w.FullRfcModernLg01 AS Word, w.SoundFileWordIdentifierText, REGEXP_REPLACE(s.urls, '.*/(.*?)/(.*?)\\\\.(mp3|ogg).*', 'https://cdstar.shh.mpg.de/bitstreams/\\\\1/\\\\2.\\\\3') AS url FROM Languages_$dt_arr[0] AS l, Transcriptions_$dt_arr[0] AS t, Words_$dt_arr[0] AS w, soundfiles AS s WHERE t.LanguageIx = $dt_arr[1] AND t.LanguageIx = l.LanguageIx AND w.IxElicitation = ".substr($dt_arr[2], 0, -1)." AND t.IxMorphologicalInstance = ".substr($dt_arr[2], -1)." AND t.IxElicitation = w.IxElicitation AND w.IxMorphologicalInstance = t.IxMorphologicalInstance AND s.LanguageIx = l.LanguageIx AND s.AlternativeLexemIx = t.AlternativeLexemIx AND s.AlternativePhoneticRealisationIx = t.AlternativePhoneticRealisationIx AND s.IxElicitation = t.IxElicitation AND s.IxMorphologicalInstance = t.IxMorphologicalInstance ORDER BY t.IxMorphologicalInstance, t.AlternativeLexemIx, t.AlternativePhoneticRealisationIx";
    $set = static::fetchAll($q);
