...
<td valign="top" style="padding-left:20px">
  ...
  <div class="modal-footer">
    ...
    if(isset($_GET['d'])){
      $th_map = array(
        'IxMorphologicalInstance' => 'IxMorph',
        'AlternativePhoneticRealisationIx' => 'AltPhon',
        'AlternativeLexemIx' => 'AltLex',
        'SpellingAltv1' => 'Spell1',
        'SpellingAltv2' => 'Spell2',
        'NotCognateWithMainWordInThisFamily' => 'NotCog',
        'CommonRootMorphemeStructDifferent' => 'ComRootDiff',
        'DifferentMeaningToUsualForCognate' => 'DiffMng',
        'ActualMeaningInThisLanguage' => 'ActMng',
        'OtherLexemeInLanguageForMeaning' => 'OtherLex',
        'RootIsLoanWordFromKnownDonor' => 'RootIsLoan',
        'RootSharedInAnotherFamily' => 'RootShared',
        'IsoCodeKnownDonor' => 'IsoDonor',
        'DifferentMorphemeStructureNote' => 'DiffMorphNote',
        'OddPhonology' => 'OddPhon',
        'OddPhonologyNote' => 'OddPhonNote',
        'UsageNote' => 'Note',
    ...
    <div style="margin:10px;">
      ...
      <script type="application/javascript">
        ...
        $(document).ready(function(){
          ...
          var table = $('table.display').DataTable({paging: false, ordering: false, searching: false});
          ...
          table.on('change', 'input.SpellingAltv2', function(){$(this).changeInRow();});
          table.on('change', 'input.NotCognateWithMainWordInThisFamily', function(){$(this).changeInRow();});
          table.on('change', 'input.CommonRootMorphemeStructDifferent', function(){$(this).changeInRow();});
          table.on('change', 'input.DifferentMeaningToUsualForCognate', function(){$(this).changeInRow();});
          table.on('change', 'input.ActualMeaningInThisLanguage', function(){$(this).changeInRow();});
          table.on('change', 'input.OtherLexemeInLanguageForMeaning', function(){$(this).changeInRow();});
          table.on('change', 'input.RootIsLoanWordFromKnownDonor', function(){$(this).changeInRow();});
          table.on('change', 'input.RootSharedInAnotherFamily', function(){$(this).changeInRow();});
          table.on('change', 'input.IsoCodeKnownDonor', function(){$(this).changeInRow();});
          table.on('change', 'input.DifferentMorphemeStructureNote', function(){$(this).changeInRow();});
          table.on('change', 'input.OddPhonology', function(){$(this).changeInRow();});
          table.on('change', 'input.OddPhonologyNote', function(){$(this).changeInRow();});
          table.on('change', 'input.UsageNote', function(){$(this).changeInRow();});
          ...
          table.on('click', '.btn.save', function(){
            ...
            , Study: tr.data('study')
            , FilePath: '<?php echo $metaData['FilePathPart'].$metaData['SoundFileWordIdentifierText'] ?>'
            };
            var fields = {};
